...
**`try_book_court` return values:** `(booked: bool, court_number: str|None, slot_open: bool, fail_reason: str|None)`. `slot_open=False` means the `Réserver` button was absent for that specific court/time (either the day's window isn't open yet, or that slot just isn't offered); `slot_open=True` means the button was present but booking didn't complete. `slot_open=False` always means "move on to the next option," never "abort the round." `fail_reason` is `None` on success, else a short code distinguishing *why* it failed: `"taken"` (Réserver was accepted then rejected — lost a race for a genuinely scarce slot), `"player_select_failed: ..."` (second-player contact match failed — usually a stale/wrong email in `KNOWN_PLAYERS`/`weekday_players` vs. the account's actual saved contact on the site), `"payment_required"` (the checkout reached a payment step asking for a real, non-zero charge — the site's own `"Aucun paiement requis"` text is absent — and the bot has no card on file, so it stops rather than submit a blank card form), `"caisse_unavailable: ..."`, `"wizard_incomplete"`, or `"no_confirmation"`. This exists because collapsing every non-"taken" failure into a generic "not available" message once hid a real config bug (a typo'd email) behind what looked like ordinary unavailability — see `run_account`'s `blocked_attempts`/`payment_attempts` split below.

**Session check:** `check_session_valid` loads `/org/4866/client` and waits for the account's `display_name` to be visible. The locator must stay restricted to visible matches (`text=<name> >> visible=true`): the page also renders the name in a hidden header inside the user-menu dropdown, which comes first in DOM order. Taking plain `.first` picked that hidden element and misreported live sessions as expired (2026-09-21 to 09-23, for both accounts). If "session expired" alerts appear right after re-running `save_auth.py`, suspect this check before the auth file.
...
**Failure reporting:** when a round ends with the window open but nothing booked, `run_account` splits `fail_reason`s three ways before emailing/logging: `blocked_attempts` (anything other than `"taken"`/`"payment_required"` — a real config or site bug) triggers a `⚠️ Blocked by an error` email listing the specific reason per court, so it's never misreported as plain unavailability. `payment_attempts` (`"payment_required"`) triggers a `💳 Manual payment needed` email with the direct booking URL for each such slot. Only when every failure was genuine contention (`"taken"`) does it send the plain `❌ All slots taken` email. `booking_history.csv`'s `notes` column carries the same distinction.
